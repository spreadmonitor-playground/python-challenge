...
let regex = try! NSRegularExpression(pattern: "[^A-Z][A-Z]{3}([a-z]){1}[A-Z]{3}[^A-Z]")
let results = regex.matches(in: gibberish, range: NSRange(gibberish.startIndex..., in: gibberish))
  .map { String(gibberish[Range($0.range, in: gibberish)!]) }
